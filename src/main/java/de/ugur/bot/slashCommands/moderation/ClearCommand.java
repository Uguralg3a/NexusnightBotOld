...
public class ClearCommand extends ListenerAdapter {
    ...
    public void prune(SlashCommandInteractionEvent event)
    {
        ...
        int amount = amountOption == null
                ? 100 // default 100
                : (int) Math.min(200, Math.max(2, amountOption.getAsInt())); // enforcement: must be between 2-200
        String userId = event.getUser().getId();
        event.reply("Bist du sicher, dass du " + amount + " Nachrichten löschen willst?") // prompt the user with a button menu
                .addActionRow(// this means "<style>(<id>, <label>)", you can encode anything you want in the id (up to 100 characters)
                        Button.danger(userId + ":delete", "Nein"),
                        Button.success(userId + ":clear:" + amount, "Ja")) // the first parameter is the component id we use in onButtonInteraction above
                .queue();
    }
