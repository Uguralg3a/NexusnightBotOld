...
public class TeamlerReport extends ListenerAdapter {

    @Override
    public void onButtonInteraction(ButtonInteractionEvent event) {
        event.deferEdit().queue();
        if (event.getButton().getId().equals("teamlerreport")) {
            ...
            if (!roles.contains("❌ × TicketBan")) {
                ...
                guild.createTextChannel("teamler-report-von-" + event.getUser().getName(), guild.getCategoryById("1001577966435827862"))
                ...
                teamEmbed.setDescription("Typ: Teamler Report");
